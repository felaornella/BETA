...
class Organizacion(db.Model):
    ...
    def __init__(self, email, nombre, geo_lat, geo_long, instagram, telefono, descripcion_breve, link_donacion, 
                dni_responsable, nombre_responsable, apellido_responsable, fecha_creacion, password):
        self.nombre = nombre
    ...
    def allEnabledCerca(geo_lat,geo_long):
        ...
        for each in organizaciones:
            each.distancia= Organizacion.calcularDistanciaEnKm([float(each.geo_lat),float(each.geo_long)],[float(geo_lat),float(geo_long)])
    ...
    def __repr__(self):
        return '<Organizacion {0} {1}>'.format(self.nombre, self.descripcion_breve, self.link_donacion, self.dni_responsable, self.nombre_responsable, self.apellido_responsable, self.fecha_creacion)
    ...
    def buscarSimilitud(info):
        organizaciones = Organizacion.query.all()
        return [org.to_dict() for org in list(
                                            filter(
                                                lambda o: (
                                                    (
                                                        info in o.nombre or 
                                                        info in o.nombre_responsable or 
                                                        info in o.apellido_responsable or 
                                                        info in o.padre.email[0:o.padre.email.index("@")]
                                                    ) and o.padre.enabled
                                                     ),organizaciones
                                                )
                                            )]
